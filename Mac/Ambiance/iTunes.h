...
@class iTPrintSettings, iTApplication, iTItem, iTArtwork, iTEncoder, iTEQPreset, iTPlaylist, iTAudioCDPlaylist, iTDevicePlaylist, iTLibraryPlaylist, iTRadioTunerPlaylist, iTSource, iTTrack, iTAudioCDTrack, iTDeviceTrack, iTFileTrack, iTSharedTrack, iTURLTrack, iTUserPlaylist, iTFolderPlaylist, iTVisual, iTWindow, iTBrowserWindow, iTEQWindow, iTPlaylistWindow;

enum iTEKnd {
	iTEKndTrackListing = 'kTrk' /* a basic listing of tracks within a playlist */,
	iTEKndAlbumListing = 'kAlb' /* a listing of a playlist grouped by album */,
	iTEKndCdInsert = 'kCDi' /* a printout of the playlist for jewel case inserts */
};
typedef enum iTEKnd iTEKnd;

enum iTEnum {
	iTEnumStandard = 'lwst' /* Standard PostScript error handling */,
	iTEnumDetailed = 'lwdt' /* print a detailed report of PostScript errors */
};
typedef enum iTEnum iTEnum;

enum iTEPlS {
	iTEPlSStopped = 'kPSS',
	iTEPlSPlaying = 'kPSP',
	iTEPlSPaused = 'kPSp',
	iTEPlSFastForwarding = 'kPSF',
	iTEPlSRewinding = 'kPSR'
};
typedef enum iTEPlS iTEPlS;
...
enum iTESrc {
	...
	iTESrcMP3CD = 'kMCD',
	iTESrcDevice = 'kDev',
	iTESrcRadioTuner = 'kTun',
	iTESrcSharedLibrary = 'kShd',
	iTESrcUnknown = 'kUnk'
};
typedef enum iTESrc iTESrc;

enum iTESrA {
	iTESrAAlbums = 'kSrL' /* albums only */,
	iTESrAAll = 'kAll' /* all text fields */,
	iTESrAArtists = 'kSrR' /* artists only */,
	iTESrAComposers = 'kSrC' /* composers only */,
	iTESrADisplayed = 'kSrV' /* visible text fields */,
	iTESrASongs = 'kSrS' /* song names only */
};
typedef enum iTESrA iTESrA;
...
enum iTESpK {
	...
	iTESpKMovies = 'kSpI',
	iTESpKMusic = 'kSpZ',
	iTESpKPartyShuffle = 'kSpS',
	iTESpKPodcasts = 'kSpP',
	iTESpKPurchasedMusic = 'kSpM',
	iTESpKTVShows = 'kSpT'
};
typedef enum iTESpK iTESpK;

enum iTEVdK {
	iTEVdKNone = 'kNon' /* not a video or unknown video kind */,
	iTEVdKMovie = 'kVdM' /* movie track */,
	iTEVdKMusicVideo = 'kVdV' /* music video track */,
	iTEVdKTVShow = 'kVdT' /* TV show track */
};
typedef enum iTEVdK iTEVdK;

enum iTERtK {
	iTERtKUser = 'kRtU' /* user-specified rating */,
	iTERtKComputed = 'kRtC' /* iTunes-computed rating */
};
typedef enum iTERtK iTERtK;
...
@interface iTPrintSettings : SBObject
...
@property (readonly) iTEnum errorHandling;  // how errors are handled
...
- (void) printPrintDialog:(BOOL)printDialog withProperties:(iTPrintSettings *)withProperties kind:(iTEKnd)kind theme:(NSString *)theme;  // Print the specified object(s)
...
@interface iTApplication : SBApplication
...
@property (copy) iTEncoder *currentEncoder;  // the currently selected encoder (MP3, AIFF, WAV, etc.)
@property (copy) iTEQPreset *currentEQPreset;  // the currently selected equalizer preset
@property (copy, readonly) iTPlaylist *currentPlaylist;  // the playlist containing the currently targeted track
...
@property (copy, readonly) iTTrack *currentTrack;  // the current targeted track
@property (copy) iTVisual *currentVisual;  //  the currently selected visual plug-in
...
@property (readonly) iTEPlS playerState;  // is iTunes stopped, paused, or playing?
...
@property iTEVSz visualSize;  // the size of the displayed visual

- (void) printPrintDialog:(BOOL)printDialog withProperties:(iTPrintSettings *)withProperties kind:(iTEKnd)kind theme:(NSString *)theme;  // Print the specified object(s)
- (void) run;  // run iTunes
- (void) quit;  // quit iTunes
- (iTTrack *) add:(NSArray *)x to:(SBObject *)to;  // add one or more files to a playlist
- (void) backTrack;  // reposition to beginning of current track or go to previous track if already at start of current track
- (iTTrack *) convert:(NSArray *)x;  // convert one or more files or tracks
...
@property (copy, readonly) iTPlaylist *parent;  // folder which contains this playlist (if any)
...
@property iTERpt songRepeat;  // playback repeat mode
@property (readonly) iTESpK specialKind;  // special playlist kind
...
- (iTTrack *) searchFor:(NSString *)for_ only:(iTESrA)only;  // search a playlist for tracks matching the search string. Identical to entering search text in the Search field in iTunes.

@end

// a playlist representing an audio CD
@interface iTAudioCDPlaylist : iTPlaylist
...
@interface iTDevicePlaylist : iTPlaylist
...
@interface iTLibraryPlaylist : iTPlaylist
...
@interface iTRadioTunerPlaylist : iTPlaylist
...
@property (readonly) iTERtK albumRatingKind;  // the rating kind of the album rating for this track
...
@property (readonly) iTERtK ratingKind;  // the rating kind of this track
...
@property iTEVdK videoKind;  // kind of video track
...
@interface iTAudioCDTrack : iTTrack
...
@interface iTDeviceTrack : iTTrack


@end

// a track representing an audio file (MP3, AIFF, etc.)
@interface iTFileTrack : iTTrack
...
@interface iTSharedTrack : iTTrack


@end

// a track representing a network stream
@interface iTURLTrack : iTTrack
...
@interface iTUserPlaylist : iTPlaylist
...
@interface iTFolderPlaylist : iTUserPlaylist


@end

// a visual plug-in
@interface iTVisual : iTItem


@end

// any window
@interface iTWindow : iTItem
...
@interface iTBrowserWindow : iTWindow

@property BOOL minimized;  // is the small player visible?
@property (copy, readonly) SBObject *selection;  // the selected songs
@property (copy) iTPlaylist *view;  // the playlist currently displayed in the window


@end

// the iTunes equalizer window
@interface iTEQWindow : iTWindow
...
@interface iTPlaylistWindow : iTWindow

@property (copy, readonly) SBObject *selection;  // the selected songs
@property (copy, readonly) iTPlaylist *view;  // the playlist displayed in the window
